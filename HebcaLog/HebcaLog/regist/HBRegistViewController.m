...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField.tag == 0) {
        [self.passswordTF becomeFirstResponder];
    }
    else if (textField.tag == 1) {
        [self.unitIdTF becomeFirstResponder];
    }
    else {
...
{
    int padding = SYSTEM_VERSION_HIGHER(8.0) ? 70 : 30;
    
    CGRect frame = textField.frame;
    CGFloat keyboardHeight = 220;
    int offset = keyboardHeight - ([UIScreen mainScreen].bounds.size.height - frame.origin.y - frame.size.height);
    ...
    if(offset > 0) {
        self.view.frame = CGRectMake(0.0f, -offset, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
    }
