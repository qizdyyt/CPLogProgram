...
static NSInteger gMailTemplateType;
...
{
    gMailTemplateType = mailTemplateType;
}

UITextView *gMailUITextView = nil;
UIWebView *gMailUIWebView = nil;
- (void)setMailUITextView:(UITextView*)mailUITextView
{
    gMailUITextView = mailUITextView;
}

- (void)setMailUIWebView:(UIWebView*)mailUIWebView
{
    gMailUIWebView = mailUIWebView;
}
...
- (void)addKeyboardCompletionHandler:(DAKeyboardDidCompleteBlock)actionHandler
{
...
- (void)addKeyboardControl:(BOOL)panning actionHandler:(DAKeyboardDidMoveBlock)actionHandler
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(responderDidEndEditing:)
...
- (void)responderDidEndEditing:(NSNotification *)notification
{
    // Grab the active input, it will be used to find the keyboard view later on
    self.keyboardActiveInput = nil;
}
...
- (void)inputKeyboardWillShow:(NSNotification *)notification
{
    ...
    UIViewAnimationOptions keyboardTransitionAnimationCurve;
    ...
    [UIView animateWithDuration:keyboardTransitionDuration
                     ...
                     completion:^(BOOL finished){
                         if(self.completionBlock)
                             self.completionBlock(finished, YES);
                     }];
}

- (void)inputKeyboardDidShow:(NSNotification *)notification
{
    if (nil == self.keyboardActiveInput) {
        if (gMailTemplateType == 0) {
            self.keyboardActiveInput = gMailUITextView;
        } else {
            self.keyboardActiveInput = gMailUIWebView;
        }
    }
