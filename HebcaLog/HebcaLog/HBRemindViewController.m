...
{
    [self.view addSubview:emptyRemindView];
    [self.view addSubview:indicatorView];
    [self.view addSubview:emptyDescription];
}
...
- (void)addSubViewWithRemindIndex:(NSInteger)index
{
    ...
    //每个提醒的view
    CGRect itemFrame = CGRectMake(8, _lastViewBottom+8, self.view.frame.size.width-16, 72 + labelsize.height);
    ...
    remindItemView.layer.borderColor = [[UIColor grayColor] CGColor];
    
    //添加control，实现点击view打开编辑视图
    UIControl *control = [[UIControl alloc] initWithFrame:remindItemView.bounds];
    ...
    UIButton *deleteBtn = [[UIButton alloc] initWithFrame:CGRectMake(itemFrame.size.width - 30, 36, 30, 30)];
    deleteBtn.tag = index;
    [deleteBtn setImage:image forState:UIControlStateNormal];
    [deleteBtn addTarget:self action:@selector(deleteRemindItem:) forControlEvents:UIControlEventTouchDown];
    [remindItemView addSubview:deleteBtn];
    
    [self.view addSubview:remindItemView];
    [self.view sendSubviewToBack:remindItemView];
    
    _lastViewBottom = itemFrame.origin.y + itemFrame.size.height + 8;
...
- (IBAction)deleteRemindItem:(id)sender
{
